...
import { PostSearchNtrmInLineTestSummaryReportRequest } from 'src/app/api-services/lab-tests/ntrm-in-line-test-report-api/requests/post-search-ntrm-in-line-test-summary-report-request';
...
@Component({
  selector: 'app-ntrm-in-line-report-summary',
  templateUrl: './ntrm-in-line-report-summary.component.html'
})
export class NtrmInLineReportSummaryComponent extends BaseComponent {
	...
	constructor(private ntrmInLineTestReportApiService: NtrmInLineTestReportApiService,
	  ...
	  this.config = BaseReportConfiguration.initialize('Ntrm in-line tests report summary', 'Export', true, formFilter);
	}

	public export(searchCriteria: RequestNtrmInLineTestReportSearchCriteria) {
	  this.loading(true);
	  const request = new PostSearchNtrmInLineTestSummaryReportRequest(searchCriteria);
	  this.ntrmInLineTestReportApiService.summaryReport(request).pipe(
		tap((x) => {
		  const blob = new Blob([x], { type: 'application/xlsx' });
		  saveAs(blob, 'ntrm_in_line_report_summary.xlsx');
		}),
